...
import com.amazonaws.services.schemaregistry.utils.apicurio.syntax2.customoptions.TestSyntax2CustomOptions
...
import com.amazonaws.services.schemaregistry.utils.apicurio.syntax3.customoptions.TestSyntax3CustomOptions
...
// Upstream test names start with a capital: kept as they are, so the reports stay comparable.
@Suppress("ktlint:standard:function-naming")
class FileDescriptorUtilsTest {
    @Test
    fun fileDescriptorToProtoFile_ParsesJsonNameOptionCorrectly() {
        val fileDescriptor = TestOrderingSyntax2.getDescriptor().file
        val expectedFieldWithJsonName = "required string street = 1 [json_name = \"Address_Street\"];\n"
        val expectedFieldWithoutJsonName = "optional int32 zip = 2 [deprecated = true];\n"

        val protoFile = FileDescriptorUtils.fileDescriptorToProtoFile(fileDescriptor.toProto())

        val actualSchema = protoFile.toSchema()
    ...
    fun ParsesFileDescriptorsAndRawSchemaIntoCanonicalizedForm_Accurately(fileDescriptor: Descriptors.FileDescriptor) {
        val fileDescriptorProto = fileDescriptor.toProto()
        val actualSchema = FileDescriptorUtils.fileDescriptorToProtoFile(fileDescriptorProto).toSchema()

        val fileName = fileDescriptorProto.name
        val expectedSchema = ProtobufTestCaseReader.getRawSchema(fileName)

        // Convert to Proto and compare
        val expectedFileDescriptorProto = schemaTextToFileDescriptor(expectedSchema, fileName).toProto()
        val actualFileDescriptorProto = schemaTextToFileDescriptor(actualSchema, fileName).toProto()

        assertEquals(expectedFileDescriptorProto, actualFileDescriptorProto, fileName)
        // We are comparing the generated fileDescriptor against the original fileDescriptorProto generated by the
        // Protobuf compiler.
    ...
    @Test
    fun ParsesSchemasWithNoPackageNameSpecified() {
        val schemaDefinition =
            "import \"google/protobuf/timestamp.proto\"; message Bar {optional google.protobuf.Timestamp c = 4; " +
                "required int32 a = 1; optional string b = 2; }"
        val actualFileDescriptorProto =
            schemaTextToFileDescriptor(schemaDefinition, "anyFile.proto").toProto().toString()
    ...
    private fun schemaTextToFileDescriptor(
        schema: String,
        fileName: String,
    ): Descriptors.FileDescriptor {
        val protoFileElement = ProtoParser.parse(FileDescriptorUtils.DEFAULT_LOCATION, schema)
        return FileDescriptorUtils.protoFileToFileDescriptor(
            schema,
            fileName,
            Optional.ofNullable(protoFileElement.packageName),
        )
    }

    @ParameterizedTest
    @MethodSource("testProtoFileProviderForJsonName")
    fun ParsesFileDescriptorsAndRawSchemaIntoCanonicalizedForm_ForJsonName_Accurately(
    ...
    companion object {
        @JvmStatic
        fun testProtoFileProvider(): Stream<Arguments> = Stream
            .of(
