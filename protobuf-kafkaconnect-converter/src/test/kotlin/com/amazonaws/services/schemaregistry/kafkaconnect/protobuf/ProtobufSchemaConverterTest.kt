...
class ProtobufSchemaConverterTest {
    ...
    @Test
    fun fromConnectData_preservesStructWireFormatWithoutSchemaParameters() {
        val childSchemaName = "$AUTOGENERATED_PACKAGE.Parent.Child"
        val childSchemaWithoutParameters =
            SchemaBuilder
                .struct()
                .name(childSchemaName)
                .field("value", Schema.STRING_SCHEMA)
                .build()
        val childSchemaWithEmptyParameters =
            SchemaBuilder
                .struct()
                .name(childSchemaName)
                .parameters(emptyMap())
                .field("value", Schema.STRING_SCHEMA)
                .build()
        val parentSchemaWithoutParameters =
            SchemaBuilder
                .struct()
                .name("Parent")
                .field("child", childSchemaWithoutParameters)
                .build()
        val parentSchemaWithEmptyParameters =
            SchemaBuilder
                .struct()
                .name("Parent")
                .field("child", childSchemaWithEmptyParameters)
                .build()
        val parentDataWithoutParameters =
            Struct(parentSchemaWithoutParameters)
                .put("child", Struct(childSchemaWithoutParameters).put("value", "test-value"))
        val parentDataWithEmptyParameters =
            Struct(parentSchemaWithEmptyParameters)
                .put("child", Struct(childSchemaWithEmptyParameters).put("value", "test-value"))

        val argumentCaptor = argumentCaptor<Message>()
        doReturn(ByteArray(0)).whenever(serializer).serialize(eq(TOPIC_NAME), any())

        protobufSchemaConverter.fromConnectData(
            TOPIC_NAME,
            parentSchemaWithEmptyParameters,
            parentDataWithEmptyParameters,
        )
        protobufSchemaConverter.fromConnectData(
            TOPIC_NAME,
            parentSchemaWithoutParameters,
            parentDataWithoutParameters,
        )

        verify(serializer, times(2)).serialize(eq(TOPIC_NAME), argumentCaptor.capture())
        val messageWithEmptyParameters = argumentCaptor.allValues[0]
        val messageWithoutParameters = argumentCaptor.allValues[1]
        assertEquals(
            messageWithEmptyParameters.descriptorForType.toProto(),
            messageWithoutParameters.descriptorForType.toProto(),
        )
        assertTrue(messageWithEmptyParameters.toByteArray().contentEquals(messageWithoutParameters.toByteArray()))

        val childMessage =
            messageWithoutParameters.getField(
                messageWithoutParameters.descriptorForType.findFieldByName("child"),
            ) as Message
        assertEquals(
            "test-value",
            childMessage.getField(childMessage.descriptorForType.findFieldByName("value")),
        )
    }
    ...
    @Test
    fun nestedOneof_roundTripsFromConnectThroughProtobufAndBack() {
        val method =
            SchemaBuilder
                .struct()
                .name("method")
                .field("card", SchemaBuilder.string().optional().build())
                .field("voucher", SchemaBuilder.int32().optional().build())
                .parameter(PROTOBUF_TYPE, PROTOBUF_ONEOF_TYPE)
                .optional()
                .build()
        val payment =
            SchemaBuilder
                .struct()
                .name("$AUTOGENERATED_PACKAGE.NestedOneofType.NestedOneofType.Payment")
                .field("reference", Schema.STRING_SCHEMA)
                .field("method", method)
                .build()
        val connectSchema =
            SchemaBuilder
                .struct()
                .name("NestedOneofType")
                .field("payment", payment)
                .build()
        val connectData =
            Struct(connectSchema).put(
                "payment",
                Struct(payment)
                    .put("reference", "INV-1")
                    .put("method", Struct(method).put("card", "4111")),
            )

        val protobufSchema = ConnectSchemaToProtobufSchemaConverter().convert(connectSchema)
        ...
        val roundTrippedPayment = roundTrippedSchema.field("payment").schema()
        assertEquals(listOf("reference", "method"), roundTrippedPayment.fields().map { it.name() })
        assertEquals(
            PROTOBUF_ONEOF_TYPE,
            roundTrippedPayment.field("method").schema().parameters()[PROTOBUF_TYPE],
        )

        val roundTrippedData =
            ProtobufDataToConnectDataConverter().toConnectData(protobufData, roundTrippedSchema) as Struct
        assertDoesNotThrow { roundTrippedData.validate() }
        assertEquals("4111", roundTrippedData.getStruct("payment").getStruct("method").get("card"))
    }
