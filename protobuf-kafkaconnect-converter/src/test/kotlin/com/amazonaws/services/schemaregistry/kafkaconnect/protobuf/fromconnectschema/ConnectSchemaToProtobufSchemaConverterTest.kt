...
class ConnectSchemaToProtobufSchemaConverterTest {
    ...
    @Test
    fun fromConnectSchema_structFieldNameThatIsNotAnIdentifier_roundTripsThroughProtobufAndBack() {
        val after =
            SchemaBuilder(Schema.Type.STRUCT)
                .name("$AUTOGENERATED_PACKAGE.Envelope.Envelope.After")
                .field("id", Schema.INT32_SCHEMA)
                .build()
        val connectSchema =
            SchemaBuilder(Schema.Type.STRUCT)
                .name("Envelope")
                .field("after.state", after)
                .build()
        val connectData = Struct(connectSchema).put("after.state", Struct(after).put("id", 7))
        ...
        assertEquals(listOf("after_state"), roundTrippedSchema.fields().map { it.name() })
        assertEquals(7, roundTrippedData.getStruct("after_state").get("id"))
    }

    @Test
    fun fromConnectSchema_mapFieldNameThatIsNotAnIdentifier_roundTripsThroughProtobufAndBack() {
        val connectSchema =
            SchemaBuilder(Schema.Type.STRUCT)
                .name("Envelope")
                .field("ts.ms", SchemaBuilder.map(Schema.STRING_SCHEMA, Schema.INT32_SCHEMA).build())
                .build()
        val connectData = Struct(connectSchema).put("ts.ms", mapOf("a" to 1))
        ...
        assertEquals(listOf("ts_ms"), roundTrippedSchema.fields().map { it.name() })
        assertEquals(mapOf("a" to 1), roundTrippedData.get("ts_ms"))
    }
    ...
    @ParameterizedTest
    @ValueSource(strings = ["mapping", "ts.ms", "ts_ms", "map-field", "2nd map"])
    fun toMapEntryName_agreesOnAName_whetherOrNotItWasSanitizedFirst(name: String) {
        assertEquals(
            ProtobufSchemaConverterUtils.toMapEntryName(name),
            ProtobufSchemaConverterUtils.toMapEntryName(ProtobufSchemaConverterUtils.toValidIdentifier(name)),
        )
    }
