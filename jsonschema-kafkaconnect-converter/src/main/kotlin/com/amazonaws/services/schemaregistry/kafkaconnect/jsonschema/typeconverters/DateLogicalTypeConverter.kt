...
class DateLogicalTypeConverter : TypeConverter {
    override fun toJson(
        schema: Schema?,
        value: Any?,
        ...
        return TypeConverter.JSON_NODE_FACTORY.numberNode(Date.fromLogical(schema, value))
    }
    ...
    override fun toConnect(
        ...
        if (!value.isInt()) {
            throw DataException(
                "Invalid type for Date, underlying representation should be integer but was " + value.nodeType,
            )
        }
    ...
    override fun toConnectSchema(
        ...
        jsonSchemaDataConfig: JsonSchemaDataConfig,
    ): SchemaBuilder = throw DataException("Invalid type for Date, underlying type should be integer")
}
