...
class BooleanTypeConverter : TypeConverter {
    ...
    ): JsonNode = TypeConverter.JSON_NODE_FACTORY.booleanNode(value as Boolean)

    override fun toJsonSchema(
        schema: Schema,
        unprocessedProperties: MutableMap<String, Any>,
        jsonSchemaDataConfig: JsonSchemaDataConfig,
    ): org.everit.json.schema.Schema.Builder<*> = BooleanSchema.builder()

    override fun toConnect(
        schema: Schema?,
        value: JsonNode,
        jsonSchemaDataConfig: JsonSchemaDataConfig,
    ): Any = value.booleanValue()
