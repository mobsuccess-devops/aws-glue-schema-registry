...
- **`FieldDescriptor.hasOptionalKeyword()` reimplemented.** protobuf 4 made the method
  ...
  descriptor proto, and the proto2 edition read off the file's `syntax` string, which is what
  `FileDescriptor.getEdition()` derives it from. `hasPresence()` is public but is not the same
